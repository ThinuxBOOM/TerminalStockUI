...
TerminalStockUI/
  ...
  backend/                  # FastAPI: api/, analytics/, forecasting/, ai/,
                            ...
                            #   db/, observability/, workers/, auth/, tests/ (52 files,
                            #   ~710 tests — rerun `pytest --collect-only` to confirm)
                            #   api/ (14 router prefixes, ~60 operations): ai, alerts,
                            #   analytics_api, audit, backtest, cron, forecast, fx,
                            #   instruments, market_data (+ securities_router),
                            #   markets, market_index (GET /api/markets/{mic}/index),
  ...
  frontend/                 # React 18 + Vite + Tailwind (plain JSX, no TS)
    src/pages/              # 11 pages: Home, Welcome (stub), LoginStub (stub), Search,
                            #   Screener, SecurityBrief, ForecastDetails,
                            #   ProviderSettings, BacktestLab, Watchlist, NotFound
    src/components/         # 23 components: ProvenanceBadge, MarketStateBadge,
                            #   CurrencyValue, FXProvenanceBanner, CalibrationChart,
                            #   AspiChart (per-market index + Top-20 + disabled CSE card),
                            #   LiquidationPanel (per-market PROXY), ResearchSection,
                            #   AIOpinionCard, NewsPanel, MarketGraphs, ... (all .jsx)
    ...
    src/api/                # client.js (zod-validated), markets.js, aspi.js
                            ...
                            #   7 vitest files (*.test.js, node env, no DOM)
  ...
  infra/migrations/         # 0001_initial … 0005_quote_snapshots + 0006_revamp.sql
                            # + 0007_horizons.sql (+ 0006_snapshots.sql draft,
                            # superseded) + alembic.ini stub
  ...
  supabase/                 # migrations 0001_onemarket … 0007_horizons (7 files,
                            # mirrors infra/ + RLS; note 0001 naming differs) + seed.sql
  docs/                     # 14 docs: API_CONTRACT, DATA_QUALITY, DB_SCHEMA (revamp normative),
                            ...
                            #   FAIL_CLOSED_CONTRACT (normative, fail-closed)
...
- **API (~60 operations across 14 router prefixes + `/health` + `/`):** `GET /health` · `/api/instruments/search|resolve|{id}` ·
  `/api/market_data/quote|bars|chart|indicators` + `/api/securities/{id}/quote|bars` ·
  ...
  `/api/screener` · `/api/news` + `/api/news/symbol/{symbol}` · `/api/signals/top` ·
  `/api/alerts` CRUD + evaluate · `/api/providers/health|keys|budget` ·
  `/api/cron/ingest|calibrate|evaluate|snapshot|score|retention|health` (GET+POST each) ·
  `/api/fx/pairs|rate|convert|rank` ·
  ...
  Machine truth: `python scripts/verify_v1.py` checks the 14 prefixes;
...
- **Supabase:** pooled `:6543` → `DATABASE_URL` (`NullPool`, `prepare_threshold=None`);
  direct `:5432` for DDL; apply `supabase/migrations/0001_onemarket` … `0007_horizons.sql`,
  seed `seed.sql` (`AAPL`, `600519.SS`, `MC.PA`). RLS enabled, no anon policies.
...
- Contract/quality/AI/venues/fail-closed: `docs/API_CONTRACT.md` · `docs/DATA_QUALITY.md` ·
  `docs/DB_SCHEMA.md` (revamp normative) · `docs/DATA_SOURCES_FREE.md` ·
  `docs/AI_PROVIDERS.md` · `docs/SSE_NOTES.md` · `docs/EURONEXT_NOTES.md` ·
  `docs/FAIL_CLOSED_CONTRACT.md` (normative)
...
- Backend suite: `python -m pytest backend/tests --collect-only -q -p no:cacheprovider`
  → **712 tests collected across 52 `test_*.py` files** at last audit (rerun
...
- Frontend: `npm run test -- --run` → 7 test files green at audit; `npm run build`
  → clean (`vite build`). Frontend is plain `.js`/`.jsx` (no TS); `npm run typecheck`
  is an intentional no-op echo.
- Backend: `python -m pytest backend/tests --collect-only -q -p no:cacheprovider`
  → 712 tests / 52 files at audit; full run `python -m pytest backend/tests -q
  -p no:cacheprovider` is the green gate (fail-closed contract: 502/423/422, never stub 200).
- Machine gates: `python scripts/verify_v1.py` → 16 PASS, 2 PARTIAL, 0 FAIL;
  `python scripts/deploy_check.py` → 9 PASS, 0 FAIL.
- Routes/migrations: ~60 `@router` operations in `backend/api/` across 14 prefixes;
  7 migrations apply (`0007_horizons.sql` last; `0006_snapshots.sql` is a superseded draft).
