...
// Phase 4b: contract specs for the module-private normalizers.
// normalizeForecast / normalizeScreener / resolveBaseUrl are NOT exported
// from client.ts (source is read-only this phase), so these specs pin the
// exported contracts those normalizers feed — verified by reading the code:
//   normalizeForecast -> ForecastSchema (label '', quality 'U',
//     provider 'deterministic-engine', probability 0..1 required)
//   normalizeScreener -> ScreenerResponseSchema (ranking passthrough,
//     skipped preservation, count/universe defaults)
//   resolveBaseUrl   -> api.defaults.baseURL/baseURL+timeout (see client.test.ts)
