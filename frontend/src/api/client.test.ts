...
// Phase 4b: pure-logic specs (Node env, no network/DOM).
...
describe('normalizeMarketState (case/whitespace/unknown)', () => {
  ...
  it('returns null for unknown or non-string input', () => {
    ...
    expect(normalizeMarketState(undefined)).toBeNull();
...
describe('constants and client defaults', () => {
  ...
  it('timeout consts: AI/SCREENER 60s, shared axios default 15s', () => {
    ...
    expect(SCREENER_TIMEOUT_MS).toBe(60000);
    expect(api.defaults.timeout).toBe(15000);
  });

  it('Node default baseURL targets local FastAPI (resolveBaseUrl contract)', () => {
    // resolveBaseUrl is module-private; its observable output is api.defaults.baseURL.
    // With no VITE_API_BASE_URL and no browser location (Node), it is the local default.
...
describe('isFreshFxProvenance (fresh-FX gate)', () => {
  ...
  it('gates everything else', () => {
    ...
    expect(isFreshFxProvenance(undefined)).toBe(false);
  });
});
