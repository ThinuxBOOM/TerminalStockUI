-- OneMarket Analyzer — Supabase migration 0006 (Supabase-ready DDL).
--
-- Derived from infra/migrations/0006_revamp.sql (identical DDL):
-- compressed market snapshots, per-forecast accuracy, AI token ledger,
-- durable provider health history, indicator cache, plus nullable
-- user_id/tier stubs on alerts + forecasts for future auth (no FK yet).
...
--   2. psql "$DATABASE_URL" -f supabase/migrations/0006_revamp.sql
...
-- revamp readers; same UNION as infra/migrations/0006_revamp.sql).
-- Append-only: NO UNIQUE constraint (writers plain-INSERT; latest-wins reads).
...
-- Convergence guards: schema converges whichever 0006 file applies first.
...
-- accuracy_id PK, NO FK on forecast_id — in-memory records score too).
...
-- Append-only AI token usage ledger (Agent 4). Secrets MUST never be stored here.
...
-- Cached deterministic indicator payloads. Not user-scoped by design.
...
-- columns only; no backfill, no FK). Other tables stay global on purpose.
