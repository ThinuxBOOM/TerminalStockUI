...
| chart (quote+bars, stitched) | `GET /api/market_data/chart?symbol=&timeframe=&limit=` → `{quote, bars, stitched, stitched_reason, provenance}`; terminal 1d bar close/high/low set from the same call's live quote (same-date sessions only; never appends, never persists) | `backend/api/market_data.py` (`get_chart`) | `frontend/src/api/client.js` (`getChart`, SecurityBrief header+chart) |
