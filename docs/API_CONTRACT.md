...
| chart (quote+bars, stitched) | `GET /api/market_data/chart?symbol=&timeframe=&limit=` → `{quote, bars, stitched, stitched_reason, forming, provenance}`; same-date sessions update the terminal 1d bar in place, newer sessions append an honest forming bar (`forming=true`, every field from the quote; weekend/holiday stamps and missing session opens decline); never persists | `backend/api/market_data.py` (`get_chart`) | `frontend/src/api/client.js` (`getChart`, SecurityBrief header+chart) |
